...
//                HomePageList("Popular Movies", popularMovies),
    ...
    open fun loadFromImdb(imdb: String): LoadResponse? {
        return loadFromImdb(imdb, emptyList())
    }

    open fun loadFromTmdb(tmdb: Int): LoadResponse? {
        return loadFromTmdb(tmdb, emptyList())
    }
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        return if (useMetaLoadResponse) {
            ...
            loadFromTmdb(id, emptyList<TvSeason>())?.let { return it }
            if (isTvSeries) {
                tmdb.tvService().externalIds(id, null).awaitResponse().body()?.imdb_id?.let { imdb ->
                    val fromImdb = loadFromImdb(imdb, emptyList<TvSeason>())
                    val result = if (fromImdb == null) {
                        val details = tmdb.tvService().tv(id, "en-US").awaitResponse().body()
                        loadFromImdb(imdb, details?.seasons ?: emptyList<TvSeason>())
                            ?: loadFromTmdb(id, details?.seasons ?: emptyList<TvSeason>())
                    } else fromImdb

                    result
                }
            } else {
                tmdb.moviesService().externalIds(id, null).awaitResponse()
                    .body()?.imdb_id?.let { imdb -> loadFromImdb(imdb, emptyList<TvSeason>()) }
            }
        }
    }
