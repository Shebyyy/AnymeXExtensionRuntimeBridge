...
open class TraktProvider : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        val apiResponse = getApi("${request.data}?extended=full&page=$page")
    ...
    override suspend fun search(query: String, page: Int): SearchResponseList? {
        val apiResponse =
            getApi("$traktApiUrl/search/movie,show?extended=full&limit=20&page=$page&query=$query")
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val resActor =
            getApi("$traktApiUrl/$moviesOrShows/${mediaDetails?.ids?.trakt}/people?extended=full")
        ...
        val resRelated =
            getApi("$traktApiUrl/$moviesOrShows/${mediaDetails?.ids?.trakt}/related?extended=full&limit=20")
        ...
        if (data.type == TvType.Movie) {
            ...
            val resSeasons =
                getApi("$traktApiUrl/shows/${mediaDetails?.ids?.trakt.toString()}/seasons?extended=full,episodes")
    ...
    private suspend fun getApi(url: String): String {
        ...
        if (!response.isSuccessful) {
            // Trakt returns plain-text errors (e.g. "Forbidden") on non-2xx
            // Return an empty JSON collection so callers don't crash on parse
            val body = response.text
            val code = response.code
            System.err.println("[TraktProvider] HTTP $code for $url — $body")
            return if (url.contains("/seasons") || url.contains("/search") || url.contains("/related") || url.contains("/trending") || url.contains("/popular")) "[]" else "{}"
        }
